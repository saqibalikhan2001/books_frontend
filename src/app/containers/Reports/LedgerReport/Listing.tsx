...
export const LedgerReportListing = ({ listing, setparam, pagination }) => {
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/ledger/pdf?download=true&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name} General Ledger Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.length > 0 && <LedgerReportsTable transactions={listing} />}
          </div>
        </div>
      )}
    </>
