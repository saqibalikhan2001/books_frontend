...
import TaxSummaryDetailsReportTable from "./TaxSummaryDetailTable";
import { CustomPaginate } from "app/shared/CustomPaginate";

const todayDate = dayjs(new Date());

export const TaxSummaryDetailsReportListing = ({
  Next,
  Prev,
  listing,
  setparam,
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/tax/details/pdf?download=true&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name} Tax By Products Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.data?.length > 0 && (
              <TaxSummaryDetailsReportTable transactions={listing} />
            )}
          </div>
        </div>
      )}
