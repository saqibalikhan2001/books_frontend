...
import TaxSummaryDetailsSupplierTable from "./TaxSummaryDetailsSupplierTable";

const todayDate = dayjs(new Date());

export const TaxSummaryDetailsSupplierListing = ({
  listing,
  setparam,
  pagination,
  Prev,
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/tax_suppliers/details/pdf_time_duration?download=true&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name ?? ""} Tax By Supplier Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.data?.length > 0 && (
              <TaxSummaryDetailsSupplierTable transactions={listing} />
            )}
          </div>
        </div>
      )}
