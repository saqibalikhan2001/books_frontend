...
import TaxSummaryDetailTimeDurationTable from "./TaxSummaryDetailTimeDurationTable";

const todayDate = dayjs(new Date());

export const TaxSummaryDetailTimeDurationListing = ({
  listing,
  setparam,
  pagination,
  Prev,
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/tax/details/pdf_time_duration?view=${pagination?.pageSize}&download=true&page=${pagination?.page}&order_by=asc&sort_by=created_at&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name || ""} Tax By Products (Time Duration) Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.data?.length > 0 && (
              <TaxSummaryDetailTimeDurationTable transactions={listing} />
            )}
          </div>
        </div>
      )}
