...
import ReceivableDetailsTable from "./ReceivableDetailsTable";

const todayDate = dayjs(new Date());

export const ReceivableDetailsReportListing = ({ listing, setparam, pagination }) => {
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/receivable/details/pdf?download=true&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name} Receivable Details Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.length > 0 && <ReceivableDetailsTable transactions={listing} />}
          </div>
        </div>
      )}
    </>
