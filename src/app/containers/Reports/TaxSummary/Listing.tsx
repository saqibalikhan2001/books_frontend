...
import TaxSummaryReportTable from "./TaxSummaryTable";

const todayDate = dayjs(new Date());

export const TaxSummaryReportListing = ({ listing, setparam, pagination }) => {
  ...
  const pdfDownload = () => {
    ...
    callAxios({
      url: `/report/tax/summary/pdf?download=true&date_range=${pagination?.date_range}`,
    }).then((res) => {
      if (res) {
        const element = document.createElement("a");
        element.href = res;
        element.download = `${name} Tax Summary Report.pdf`;
        element.click();
      }
    });
  };
      ...
      {false ? (
        ...
        <div className="item_listing">
          <div className="custom-filter pr-15">
            ...
            {listing?.data?.length > 0 && <TaxSummaryReportTable transactions={listing} />}
          </div>
        </div>
      )}
    </>
